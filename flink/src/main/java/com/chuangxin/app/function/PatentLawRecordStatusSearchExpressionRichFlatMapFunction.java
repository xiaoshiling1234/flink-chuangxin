 ...
 * 需要截取保留最大的
 */
public class PatentLawRecordStatusSearchExpressionRichFlatMapFunction extends RichFlatMapFunction<String, String> {
    ...
    public void close() {
        try {
            ...
            MysqlUtil.update("task", updateInfo, "task_name='FLINK-SYNC:PATENT_LAW_RECORD_STATUS_SEARCH_EXPRESSION'");
    ...
    public void flatMap(String s, Collector<String> collector) {
        ...
        records.forEach(record -> {
            String ilsad = JSONObject.parseObject(record.toString()).getString("ilsad");
            String pdFormat = convertDateFormat(ilsad);

            try {
                if (maxDtState.value() == null) {
                    maxDtState.update(pdFormat);
                    System.out.println("最大法律公告日已更新为:" + pdFormat);
                } else {
