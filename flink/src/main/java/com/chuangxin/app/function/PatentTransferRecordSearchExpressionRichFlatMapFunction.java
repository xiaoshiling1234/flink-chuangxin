...
public class PatentTransferRecordSearchExpressionRichFlatMapFunction extends RichFlatMapFunction<String, String> {
    ...
    public void close() {
        try {
            ...
            MysqlUtil.update("task", updateInfo, "task_name='FLINK-SYNC:PATENT_TRANSFER_RECORD_SEARCH_EXPRESSION'");
    ...
    public void flatMap(String s, Collector<String> collector) {
        ...
        records.forEach(record -> {
            String pdq = JSONObject.parseObject(record.toString()).getString("pdq");
            String pdFormat = convertDateFormat(pdq);

            try {
                if (maxDtState.value() == null) {
                    maxDtState.update(pdFormat);
                    System.out.println("最大授权公布日已更新为:" + pdFormat);
                } else {
